...
import com.example.cropview.animation.SimpleValueAnimator;
import com.example.cropview.animation.SimpleValueAnimatorListener;
import com.example.cropview.animation.ValueAnimatorV14;
import com.example.cropview.animation.ValueAnimatorV8;
import com.example.cropview.callback.Callback;
...
import com.example.cropview.util.Logger;
  ...
  private void handleStyleable(Context context, AttributeSet attrs, int defStyle, float mDensity) {
    ...
    try {
      ...
      for (CropMode mode : CropMode.values()) {
        if (ta.getInt(R.styleable.scv_CropImageView_scv_crop_mode, 6) == mode.getId()) {
          mCropMode = mode;
          break;
        }
      }
      ...
      for (ShowMode mode : ShowMode.values()) {
        if (ta.getInt(R.styleable.scv_CropImageView_scv_guide_show_mode, 3) == mode.getId()) {
          mGuideShowMode = mode;
          break;
        }
      }
